...
async function onPollInvoke(
  ...
  switch (subcommand) {
    case "create":
      ...
      let rows = builders.map((b) =>
        new ActionRowBuilder<TextInputBuilder>().addComponents(b)
      );
    ...
    case "display":
      ...
      if (openPolls.totalDocs > 0) {
        let row = createPollSelectableMenu(
          openPolls.docs,
          subcommand == "display" ? DisplayPollId : EndPollId
        );
        let text =
          subcommand == "display"
        ...
        await interaction.reply({
          content: "You have no current polls.",
          ephemeral: true,
        });
      }
      break;
...
async function onInteractionCreate(interaction: Interaction<CacheType>) {
  ...
  if (interaction.isModalSubmit() && interaction.customId == PollModalId) {
    ...
    if (match) {
      ...
      let pollId = match[1];
      let choiceNumber = parseInt(match[2]);
...
export const PollCommand: Command = {
  ...
  data: new SlashCommandBuilder()
    .setName("poll")
    .setDescription(
      "Allows you to create polls that other club members can vote on."
    )
