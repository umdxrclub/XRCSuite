...
type ScannerStatus =
  ...
  | "error";
type ScannerConfig = {
  colors: Record<ScannerStatus, string | undefined>;
...
export const GatekeeperScanner: React.FC<GatekeeperScannerProps> = ({
  config,
  resolver,
}) => {
  // Swipe card
  ...
  let fullConfig: ScannerConfig = {
    ...DefaultScannerConfig,
    ...config,
  };

  // Current scanner status metadata.
  const statusTitle = fullConfig.titles[scannerStatus];
  const statusColor = fullConfig.colors[scannerStatus];
  const statusIcon = fullConfig.icons[scannerStatus];
  const statusDescription = fullConfig.descriptions[scannerStatus]
    .replace("{NAME}", checkedInMemberName)
