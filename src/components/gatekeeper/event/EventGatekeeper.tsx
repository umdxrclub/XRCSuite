...
export const EventGatekeeper: React.FC = ({ }) => {
    ...
    const EventGatekeeperResolver: GatekeeperResolver = async (method, value) => {
        var checkInURL = new URL('/api/events/' + eventId + '/checkin', window.location.origin)
    ...
    return <Box width="100%" height="100%" display="flex" flexDirection="row" id="event-gk-root">
        <RobotoLink />
        <Box flex={4} display="flex">
            { event ? <Box margin={4} flex={1}>
                <div style={{
