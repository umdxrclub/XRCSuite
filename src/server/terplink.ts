...
import { TerpLinkSchema } from "../types/TerpLinkSchema";
import {
  queryStringFromForm,
  wasRequestRedirectedTo,
...
const TERPLINK_TOKEN_URL =
  "https://terplink.umd.edu/account/login?ReturnUrl=%2Faccount%2Ftokens%3FisMobile%3Dtrue";
...
export class RosterMember {
  ...
  public async fetchEmail(): Promise<string> {
    const res = await this._axios.get(
      `https://terplink.umd.edu/actioncenter/organization/xr-club/roster/users/membercard/${this.communityId}`,
      {
        headers: {
    ...
    const email = body
      .querySelector("a[class=email]")
      ?.attributes["href"].split("mailto:")[1];
    return email!;
  }
}

export class ProspectiveRosterMember extends RosterMember {
  private _requestVerificationToken: string;
  ...
  constructor(
    ...
    super(communityId, name, axios);
    this._requestVerificationToken = requestVerificationToken;
  }
  public async approveMember() {
    await this.postWithVerificationToken(
      `https://terplink.umd.edu/actioncenter/organization/xr-club/roster/roster/approvemember/${this.communityId}`
    );
  }

  public async denyMember() {
    await this.postWithVerificationToken(
      `https://terplink.umd.edu/actioncenter/organization/xr-club/roster/roster/denymember/${this.communityId}`
    );
  }

  private async postWithVerificationToken(url: string) {
    let params = new URLSearchParams();
    params.set("__RequestVerificationToken", this._requestVerificationToken);
    return this._axios.post(url, params.toString(), {
      ...X_WWW_FORM_HEADERS_CONFIG,
    });
  }
}
...
export class TerpLink {
  ...
  constructor(axios: Axios) {
    ...
    axios.interceptors.response.use(
      this.terplinkResponseInterceptor.bind(this)
    );
  }
  ...
  private async terplinkResponseInterceptor(res: AxiosResponse<any, any>) {
    ...
    if (wasRequestRedirectedTo(reqUrl, responseUrl, TERPLINK_FEDERATION_URL)) {
      ...
      const campusForm = queryStringFromForm(
        identityRoot.querySelector("form")!
      );
      const connectRes = await this.axios.post(
        campusPostURL,
        campusForm,
        X_WWW_FORM_HEADERS_CONFIG
      );
      ...
      var oidcRes = await this.axios.post(
        connectPostUrl,
        connectQuery,
        X_WWW_FORM_HEADERS_CONFIG
      );
    ...
    // Stale token
    if (
      TERPLINK_API_URLS.find((url) => res.config.url?.startsWith(url)) &&
      res.status == 401
    ) {
      await this.getBearer();
  ...
  private async fetchRosterTable<T extends RosterMember>(
    rosterCreator: (communityId: string, title: string, body: HTMLElement) => T,
    tableUrl: string,
    name?: string
  ): Promise<T[]> {
    ...
    let extractMembers = (page: string) => {
      ...
      if (pagination) {
        ...
        if (navLinks.length > 0) {
          last = parseInt(
            navLinks[navLinks.length - 1].attributes["href"].split("Page=")[1]
          );
        }
      }
      ...
      return {
        members: matches.map((e) => {
          const communityId = e.attributes["value"];
          const title = e.attributes["title"];
          let name = title;
          if (name.startsWith("Select ")) {
            name = name.split("Select ")[1];
          }

          return rosterCreator(communityId, name.trim(), body);
        }),
        lastPage: last,
      };
    };
    ...
    let members: T[] = [...initExtraction.members, ...pageMembers.flat()];
  ...
  /**
   * Approves all prospective members.
   */
  public async approveAllProspectiveMembers() {
    var members = await this.getProspectiveRosterMembers();
    for (var member of members) {
        await member.approveMember()
        console.log("Approved " + member.name)
    }
  }

  /**
   * Gets prospective roster members.
   *
   * @param name The name to filter by.
   * @returns List of roster members
   */
  public async getProspectiveRosterMembers(): Promise<
    ProspectiveRosterMember[]
  > {
    ...
    return await this.fetchRosterTable((communityId, title, body) => {
      ...
      if (!requestVerificationToken) {
        ...
        requestVerificationToken = body.querySelectorAll(
          'input[name="__RequestVerificationToken"]'
        )[2].attributes["value"];
      }

      return new ProspectiveRosterMember(
        communityId,
        title,
        requestVerificationToken,
        this.axios
      );
    }, "https://terplink.umd.edu/actioncenter/organization/xr-club/roster/Roster/prospective");
  }
  ...
  public async getRosterMembers(name?: string): Promise<RosterMember[]> {
    return await this.fetchRosterTable(
      (communityId, title) => new RosterMember(communityId, title, this.axios),
      "https://terplink.umd.edu/actioncenter/organization/xr-club/roster",
      name
    );
  }
  ...
  public async getEmailFromCommunityId(communityId: string) {
    const res = await this.axios.get(
      `https://terplink.umd.edu/actioncenter/organization/xr-club/roster/users/membercard/${communityId}`
    );
    const body = parse(res.data);
    const email = body
      .querySelector("a[class=email]")
  ...
  public async parseEventPage(eventId: number): Promise<EventPage | undefined> {
    let page = await this.getEventPage(eventId);
    if (page) {
      let dom = parse(page);

      let accessCodeInput = dom.querySelector("#copytextAcessCode");
      if (!accessCodeInput) return;

      let accessCode = accessCodeInput.getAttribute("value");
      if (!accessCode) return;

      let locationSpan = dom.querySelectorAll(".list-item-content > span")[3];
      let location = locationSpan.innerText.trim();
  ...
  public async getEvents(organizationId: number) {
    const res = await this.axios.get(
      "https://terplink.umd.edu/api/comp-events/graphql/getEvents",
      {
        data: {
...
export class TerpLinkEvent {
  ...
  async lookupMembers(query: string): Promise<TerpLinkEventMember[]> {
    ...
    return members.map(
      (member) =>
        new TerpLinkEventMember(member.attendanceId, member.account, this)
    );
  }
   ...
   */
  async getMemberFromIssuanceId(
    issuanceId: string
  ): Promise<TerpLinkEventMember | undefined> {
    var member: TerpLinkEventMember | undefined = undefined;
    try {
      const res = await this.get(
        `member?issuanceId=${issuanceId}&skip=0&take=10`
      );
      if ((res.data.items as any[]).length > 0) {
        const tlMember = res.data.items[0] as TerpLinkSchema.Member;

        member = new TerpLinkEventMember(
          tlMember.attendanceId,
          tlMember.account,
          this
        );
      }
  ...
  async getEventAttendees(
    withLogAction?: TerpLinkSchema.AttendeeLastLogAction[],
    includeRemoved?: boolean
  ): Promise<TerpLinkEventMember[]> {
    ...
    if (withLogAction) filter = withLogAction;
    else filter = ["CheckIn"];

    // Filter attendees.
    attendees = attendees.filter(
      (attendee) =>
        filter.includes(attendee.lastLogAction) &&
        (includeRemoved || attendee.status === "Attended")
    );

    return attendees.map(
      (attendee) =>
        new TerpLinkEventMember(
          attendee.id,
          attendee.account,
          this,
          attendee.lastLogAction
        )
    );
  }
...
export class TerpLinkEventMember {
  ...
  constructor(
    id: number | null,
    account: TerpLinkSchema.Account,
    event: TerpLinkEvent,
