...
function areSetsEqual<T>(
  ...
  return (
    ...
    arr1.every((a) => arr2.some((b) => property(a) === property(b)))
  );
}
...
function deepEquality(a: any, b: any): boolean {
  ...
  let typeB = typeof b;
  let isStringObj =
    (typeA == "string" && typeB == "object") ||
    (typeA == "object" && typeB == "string");
  let isUnresolvedField =
    isStringObj &&
    ((typeA == "object" && a["id"]) || (typeB == "object" && b["id"]));
