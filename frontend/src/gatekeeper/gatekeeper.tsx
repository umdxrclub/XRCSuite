...
import { xrcv1 } from "xrc-schema/dist/api/api";
...
const STATUS_SVG: Record<GatekeeperStatus, React.FunctionComponent<React.SVGProps<
SVGSVGElement>>> = {
    "found": ProceedSVG,
...
export const Gatekeeper: React.FC = ({ children }) => {
     ...
     */
    const success = useCallback(async (text: string, result: Html5QrcodeResult) => {
        ...
        if (typeof(code.issuanceId) === "string") {
            ...
            try {
                const res = await xrcv1.post("/terplink/:eventcode/checkin", {
                    path: { eventcode: eventcode! },
                ...
                setStatus({
                    status: "found",
                    text: STATUS_DESCRIPTION["found"].replace("{NAME}", checkedInMember.name)
                });
            } catch {
                setStatus({
                    status: "error",
    ...
    return (<div className="gk-root">
        <div className="gk-event">
            ...
            <div className="gk-terplink">
                <p>Don't have a TerpLink event pass?<br/>Scan the following QR Code to create one (it's free!).</p>
